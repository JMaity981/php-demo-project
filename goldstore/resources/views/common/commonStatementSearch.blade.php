...
<div class="form-row">
	<div class="col-md-4">
		<div class="form-group">
			...
			{{-- <input type="text" class="form-control daterange" value="{{date('01/m/Y')}} - {{date('d/m/Y')}}"> --}}
			...
			<select class="single-select proprietor-name" name="propiter_name">
				<option value="" disabled selected>Select Jewellers Name / Proprietor Name</option>
				 @foreach ($data['lager'] as $key => $customer_data)
					<option value="{{ $customer_data['id'] }}">{{ $customer_data['jewelry_name'] }} / {{ $customer_data['proprietor_name'] }} </option>
				@endforeach 
			</select>
		</div>
	</div>
	@if(Request::path() == 'our-sale-and-parchase')
		<div class="col-md-2">
			<br><br>
