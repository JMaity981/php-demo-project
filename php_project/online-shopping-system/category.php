...
<div class="wrap">
    ...
    <div class="featur_box containt">
        ...
        <?php
		$i=1;
		$qry="select * from product where status=1 and cat_id='".$cat_id."'";
		if(isset($_GET['color']) && $_GET['color']!="all")
		{
			$qry.=" and prod_color='".$_GET['color']."'";
		}
		$qry.=" limit $start,8";
       ...
       <?php 
	   				$qry_string[1]="";
					$qry_string=explode('?',$_SERVER['REQUEST_URI']);
					$qry_string1=explode('&',$qry_string[1]);
					if(isset($_GET['color']))
					$qry_string[1]=$qry_string1[0]."&".$qry_string1[1];
					else
					$qry_string[1]=$qry_string1[0];
					
					$qry_pagi="select * from product where status=1 and cat_id='".$cat_id."'";
					if(isset($_GET['color']) && $_GET['color']!="all")
					{
						$qry_pagi.=" and prod_color='".$_GET['color']."'";
					}
					...
					if(($productno-1) > 1 && $val > 7)
					{
						$prev = $productno-2;
					?>
					  <a href="<?php echo $_SERVER['PHP_SELF'];?>?<?=$qry_string[1];?>&r_productno=<?php echo $prev;?>" style="text-decoration:none"><b><&nbsp;Prev</b></a>
					<?php
					}
					else if($val > 7)
					{
					...
					for($i=$s;$i<=($s+6) && $i<=$val;$i++)
					{
						...
						{
					?>
					  <a href="<?php echo $_SERVER['PHP_SELF'];?>?<?=$qry_string[1];?>&r_productno=<?php echo $i;?>" style="text-decoration:none"><b><font size="+2"><?php echo $i;?> </font></b></a>
					<?php
						}
					...
					<?php
						...
						{
					?>
					  <a href="<?php echo $_SERVER['PHP_SELF'];?>?<?=$qry_string[1];?>&r_productno=<?php echo $i;?>" style="text-decoration:none"><?php echo $i;?></a>
					<?php
						}
					}
					...
					if(($productno-1) < $val && $val > 7)
					{
						$next = $productno;
					?>
					  <a href="<?php echo $_SERVER['PHP_SELF'];?>?<?=$qry_string[1];?>&r_productno=<?php echo $next;?>" style="text-decoration:none"><b>Next&nbsp;></b></a>
					<?php
					}
					else if($val > 7)
					{
